...
export function sanitizeAuditInputs(formData) {
  ...
  // Sanitizing inactive seats count (must be a whole integer >= 0)
  let inactiveSeatsCount = 0;
  if ('inactiveSeats' in formData) {
    const parsedInactive = parseInt(formData.inactiveSeats, 10);
    inactiveSeatsCount = !isNaN(parsedInactive) && parsedInactive >= 0 ? parsedInactive : 0;
  }
