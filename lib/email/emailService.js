...
/**
 * Generates clean, premium HTML for the Demo Request scheduling confirmation email.
 * 
 * @param {Object} params
 ...
 * @returns {string} Fully structured HTML content
 */
export function generateDemoConfirmationEmailHtml({ contactName, companyName, useCase }) {
  const useCaseBlock = useCase && useCase.trim() ? `
    <div style="margin-bottom: 28px; padding: 20px; background-color: #fafafa; border: 1px solid #f3f4f6; border-radius: 6px;">
      <h3 style="margin: 0 0 10px 0; font-size: 12px; font-weight: 700; text-transform: uppercase; letter-spacing: 0.05em; color: #4b5563;">Your Notes / Use Case</h3>
      <p style="margin: 0; font-size: 13.5px; line-height: 1.6; color: #374151; white-space: pre-wrap;">${useCase.trim()}</p>
    </div>
  ` : '';
...
<head>
  <meta charset="utf-8">
  <meta name="viewport" content="width=device-width, initial-scale=1.0">
  <title>CredLens Demo Scheduled</title>
</head>
<body style="margin: 0; padding: 0; background-color: #f9fafb; font-family: -apple-system, BlinkMacSystemFont, 'Segoe UI', Roboto, Helvetica, Arial, sans-serif; -webkit-font-smoothing: antialiased;">
  <table border="0" cellpadding="0" cellspacing="0" width="100%" style="table-layout: fixed;">
    <tr>
        ...
        <table border="0" cellpadding="0" cellspacing="0" width="100%" style="max-width: 600px; background-color: #ffffff; border: 1px solid #e5e7eb; border-radius: 8px; overflow: hidden; box-shadow: 0 4px 6px -1px rgba(0, 0, 0, 0.05);">
          ...
          <tr>
            <td style="padding: 32px 32px 24px 32px; border-bottom: 1px solid #f3f4f6;">
              <table border="0" cellpadding="0" cellspacing="0" width="100%">
                <tr>
                  <td>
                    <span style="font-size: 10px; font-weight: 700; letter-spacing: 0.1em; text-transform: uppercase; color: #6b7280; font-family: monospace;">CredLens Onboarding</span>
                    <h1 style="margin: 4px 0 0 0; font-size: 20px; font-weight: 700; color: #111827;">Demo Scheduled</h1>
                  </td>
                  <td align="right">
                    <span style="display: inline-block; background-color: #ecfdf5; border: 1px solid #a7f3d0; border-radius: 4px; padding: 4px 8px; font-size: 11px; font-weight: 600; color: #047857; font-family: monospace;">REQUEST LOGGED</span>
                  </td>
                </tr>
              </table>
            </td>
          ...
          <tr>
            <td style="padding: 32px;">
              ...
              <p style="margin: 0 0 24px 0; font-size: 15px; line-height: 1.6; color: #374151;">
                Thanks for requesting a personalized spend optimization demo for <strong>${companyName}</strong>. We've logged your request and our onboarding team will reach out to you within 24 hours to coordinate a suitable time slot.
              </p>
              
              <!-- Value Prop Box -->
              <div style="margin-bottom: 28px; padding: 20px; background-color: #fafafa; border: 1px solid #f3f4f6; border-radius: 6px;">
                <h3 style="margin: 0 0 12px 0; font-size: 12px; font-weight: 700; text-transform: uppercase; letter-spacing: 0.05em; color: #4b5563;">What to expect in the demo</h3>
                
                <table border="0" cellpadding="0" cellspacing="0" width="100%">
                  <tr>
                    <td style="padding-bottom: 12px; vertical-align: top;">
                      <strong style="font-size: 13px; color: #111827; display: block;">1. Custom Audit Review</strong>
                      <span style="font-size: 12px; color: #4b5563; line-height: 1.4;">We will analyze your active tool stack configuration, pricing options, and seat layouts together.</span>
                    </td>
                  </tr>
                  <tr>
                    <td style="padding-bottom: 12px; vertical-align: top;">
                      <strong style="font-size: 13px; color: #111827; display: block;">2. Slack Integration Walkthrough</strong>
                      <span style="font-size: 12px; color: #4b5563; line-height: 1.4;">See how the CredLens Slack app provides automated alerts 14 days before a subscription renewal occurs.</span>
                    </td>
                  </tr>
                  <tr>
                    <td style="vertical-align: top;">
                      <strong style="font-size: 13px; color: #111827; display: block;">3. API Telemetry Setup</strong>
                      <span style="font-size: 12px; color: #4b5563; line-height: 1.4;">Learn how to connect your OpenAI, Anthropic, or Google API log exports securely without exposing keys.</span>
                    </td>
                  </tr>
                </table>
              </div>
              ...
              <p style="margin: 0; font-size: 14px; line-height: 1.6; color: #374151;">
                If you have any initial billing exports or stack plans you would like us to review ahead of time, feel free to reply directly to this email.
              </p>
              
              <p style="margin: 24px 0 0 0; font-size: 13.5px; line-height: 1.5; color: #4b5563;">
                Best regards,<br>
                <strong>The CredLens Onboarding Team</strong>
              </p>
            </td>
          </tr>
          ...
          <tr>
            <td style="background-color: #fafafa; border-top: 1px solid #f3f4f6; padding: 16px 32px; text-align: center;">
              <span style="font-size: 11px; font-family: monospace; color: #9ca3af;">CredLens Audit Engine &middot; Demo Pipeline</span>
            </td>
          </tr>
        </table>
      </td>
...
/**
 * Dispatches a professional confirmation email for a demo request.
 * 
 * @param {Object} params
 * @param {string} params.to
 * @param {string} params.contactName
 ...
 * @returns {Promise<{success: boolean, error?: string, data?: any}>} Delivery outcome
 */
export async function sendDemoConfirmationEmail({ to, contactName, companyName, useCase }) {
  const resend = getResendClient();
  if (!resend) {
    return {
      success: false,
  ...
  const fromEmail = process.env.RESEND_FROM_EMAIL || 'CredLens <onboarding@resend.dev>';
  const htmlContent = generateDemoConfirmationEmailHtml({ contactName, companyName, useCase });

  try {
    const { data, error } = await resend.emails.send({
      from: fromEmail,
      to: [to.trim().toLowerCase()],
      subject: `Demo Scheduled: CredLens Cost Optimization`,
      html: htmlContent,
    });

    if (error) {
      console.error('[Email Service] Resend API error sending demo confirmation:', error);
      return {
        success: false,
        error: error.message || JSON.stringify(error),
      };
    ...
    console.log(`[Email Service] Demo confirmation successfully dispatched to ${to}. ID: ${data?.id}`);
    return {
      success: true,
      data,
    };
  } catch (err) {
    console.error('[Email Service] Unexpected demo mail delivery exception:', err);
    return {
      success: false,
      error: err.message || 'An unexpected error occurred during email delivery.',
    };
