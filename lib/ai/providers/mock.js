...
export function generateMockSummary(auditData) {
  ...
  const formattedSavings = summary.formattedEstimatedSavings || `$${summary.totalEstimatedSavings}/mo`;
  const formattedOptimized = summary.formattedOptimizedSpend || `$${summary.optimizedSpendEstimate}/mo`;
  const formattedYearly = summary.formattedEstimatedYearlySavings || `$${summary.totalEstimatedSavings * 12}/yr`;
  const formattedCurrent = summary.formattedCurrentSpend || `$${summary.totalCurrentSpend}/mo`;
