...
export function generateMockSummary(auditData) {
  ...
  if (!hasSavings) {
    const zeroSavingsSummary = `An audit of the ${projectName || 'Startup'} AI stack confirms that your current subscription licenses and API configurations are fully optimized. Your active monthly baseline of ${formattedCurrent} matches your calculated target spend.`;
    return {
      ai_audit_summary: zeroSavingsSummary,
      executiveSummary: zeroSavingsSummary,
      keyInsights: [
