...
export function validateAndRepairSummary(parsed, fallbackData) {
  const aiAuditSummary = parsed?.ai_audit_summary || parsed?.executiveSummary || fallbackData.ai_audit_summary || fallbackData.executiveSummary || '';
  const result = {
    ai_audit_summary: aiAuditSummary,
    executiveSummary: aiAuditSummary,
    keyInsights: [],
