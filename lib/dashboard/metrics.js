...
import {
  detectToolOverlaps,
  deriveSmartRedundancyAlerts,
  deriveSmartRiskMatrix,
  deriveSmartInsightEvents
} from './intelligence';
...
export function deriveDashboardMetrics(audit) {
  ...
  // Use the new intelligence layer for alerts, risk matrix, and activity feed
  const detectedOverlaps = detectToolOverlaps(audit.tools || []);
  const alerts = deriveSmartRedundancyAlerts(audit);
  const riskMatrix = deriveSmartRiskMatrix(audit);
  const events = deriveSmartInsightEvents(audit, detectedOverlaps);
  ...
  return {
    ...
    riskMatrix,
    events,
    ai_audit_summary: audit.aiSummary?.ai_audit_summary || audit.aiSummary?.executiveSummary || '',
    auditMeta: {
