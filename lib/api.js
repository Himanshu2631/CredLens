...
/**
 * Submits demo scheduling request info to the database API route.
 * 
 * @param {Object} payload (contactName, email, companyName, useCase, teamSize, monthlySpend, providers)
 * @returns {Promise<Object>} Persisted DemoRequest record
 */
export async function createDemoRequest(payload) {
  const response = await fetch('/api/schedule-demo', {
    ...
    body: JSON.stringify({
      ...
      useCase: payload.useCase || '',
      teamSize: payload.teamSize ? Number(payload.teamSize) : null,
      monthlySpend: payload.monthlySpend ? Number(payload.monthlySpend) : null,
